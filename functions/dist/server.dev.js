...
var cron = require('node-cron'); // Set up SendGrid API key
...
var scheduleEmailSendingAndOTPDisabling = function scheduleEmailSendingAndOTPDisabling() {
  ...
  return regeneratorRuntime.async(function scheduleEmailSendingAndOTPDisabling$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 8:
          recipientEmails = ['lanre.mohammed23@gmail.com', 'al-hameen.mohammed@pau.edu.ng'];
        ...
        case 13:
          ...
          console.error('Error scheduling email:', _context.t0.message);
        ...
        case 25:
          ...
          console.error('Error scheduling otpDisabled:', _context.t1.message);
...
cron.schedule('* * * * * *', scheduleEmailSendingAndOTPDisabling); // Create an Express application
...
router.get('/send-email', function (req, res) {
  var recipientEmails = ['lanre.mohammed23@gmail.com', 'al-hameen.mohammed@pau.edu.ng'];
  sendEmail(recipientEmails, function (error, result) {
    if (error) {
      res.status(500).send('Error sending email: ' + error.message);
    } else {
